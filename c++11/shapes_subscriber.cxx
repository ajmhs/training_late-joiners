...
{
    dds::sub::LoanedSamples< ::ShapeTypeExtended> samples = reader.read();
    
    unsigned int value_sum = 0;
    for (auto sample : samples) {
        if (sample.info().valid()) {

            // Sum all the samples in the history queue - should be max of history-depth            
            value_sum += sample.data().shapesize();

            if (dds::sub::status::SampleState::not_read() == sample.info().state().sample_state()) {
                // Haven't seen this sample before, increment the count and output the current sum 
                ++samples_read;
                std::cout << "Processed " << samples_read << " sample" << (samples_read > 1 ? "s, ": ", ") 
                    << "sum of last " << history_depth << " values: " << value_sum << std::endl;
            }                
        }
        else {
...
void run_subscriber_application(unsigned int domain_id, unsigned int sample_count)
{
    ...
    auto historyQos = reader.qos().policy<dds::core::policy::History>();
    history_depth = historyQos.depth();
